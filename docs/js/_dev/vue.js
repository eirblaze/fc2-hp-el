...
(function (global, factory) {
...

  {
    ...
    var classify = function (str) { return str
      .replace(classifyRE, (c) => { return c.toUpperCase(); })
      .replace(/[-_]/g, '') };
  ...
  Dep.prototype.notify = function notify () {
    ...
    if (!config.async) {
      ...
      // order
      subs.sort((a, b) => { return a.id - b.id; });
    }
  ...
  function isExplicable (value) {
    ...
    return explicitTypes.some((elem) => { return value.toLowerCase() === elem; })
  }
  ...
  function isBoolean () {
    ...
    return args.some((elem) => { return elem.toLowerCase() === 'boolean'; })
  }
  ...
  function invokeWithErrorHandling (
    ...
    try {
      ...
      if (res && !res._isVue && isPromise(res) && !res._handled) {
        res.catch((e) => { return handleError(e, vm, info + " (Promise/async)"); });
        // issue #9511
...
;if (!pending) {
  ...
  let normalizeEvent = cached((name) => {
...
;toggleObserving(true)
  ...
  function resolveAsyncComponent (
    ...
    if (owner && !isDef(factory.owners)) {
      ...
      ;(owner).$on('hook:destroyed', () => { return remove(owners, owner); })
...
;this.$watch('exclude', (val) => {
  ...
  function pluckModuleFunction (
    ...
    return modules
      ? modules.map((m) => { return m[key]; }).filter((_) => { return _; })
      : []
  }
  ...
  function addHandler (
    ...
    let newHandler = rangeSetItem({ value: value.trim(), dynamic }, range)
...
;function updateStyle (oldVnode, vnode) {
  ...
  function addClass (el, cls) {
    ...
    if (el.classList) {
      if (cls.indexOf(' ') > -1) {
        cls.split(whitespaceRE).forEach((c) => { return el.classList.add(c); });
      } else {
  ...
  function removeClass (el, cls) {
    ...
    if (el.classList) {
      if (cls.indexOf(' ') > -1) {
        cls.split(whitespaceRE).forEach((c) => { return el.classList.remove(c); });
      } else {
...
;if (delayLeave) {
  ...
  var directive = {
    ...
    componentUpdated: function componentUpdated (el, binding, vnode) {
      if (vnode.tag === 'select') {
        ...
        if (curOptions.some((o, i) => { return !looseEqual(o, prevOptions[i]); })) {
          // trigger change event if
          // no matching option found for at least one value
          let needReset = el.multiple
            ? binding.value.some((v) => { return hasNoMatchingOption(v, curOptions); })
  ...
  function hasNoMatchingOption (value, options) {
    return options.every((o) => { return !looseEqual(o, value); })
  }
  ...
  let isNotTextNode = function (c) { return c.tag || isAsyncPlaceholder(c) };

  let isVShowDirective = function (d) { return d.name === 'show' };
  ...
  var TransitionGroup = {
    ...
    beforeMount: function beforeMount () {
      ...
      this._update = function (vnode, hydrating) {
        let restoreActiveInstance = setActiveInstance(this$1)
        // force removing pass
        this$1.__patch__(
          this$1._vnode,
          this$1.kept,
...
;function parseText (
  ...
  function decodeAttr (value, shouldDecodeNewlines) {
    ...
    return value.replace(re, (match) => { return decodingMap[match]; })
  }
  ...
  function parseHTML (html, options) {
    ...
    while (html) {
      ...
      if (!lastTag || !isPlainTextElement(lastTag)) {
        ...
        var rest$1 = html.replace(reStackedTag, (all, text, endTag) => {
...
;index += html.length - rest$1.length
  ...
  function parse (
    ...
    function closeElement (element) {
      ...
      element.children = element.children.filter((c) => { return !(c).slotScope; });
    ...
    if (exp) {
      {
        ...
        if (el.for) {
          let iterator = el.iterator2 || el.iterator1
  ...
  function processSlotContent (el) {
    ...
    // 2.6 v-slot syntax
    {
      if (el.tag === 'template') {
        // v-slot on <template>
        let slotBinding = getAndRemoveAttrByRegex(el, slotRE)
        if (slotBinding) {
          {
          ...
          let ref = getSlotName(slotBinding)
          var name = ref.name
        ...
        let slotBinding$1 = getAndRemoveAttrByRegex(el, slotRE)
        if (slotBinding$1) {
          {
          ...
          let slots = el.scopedSlots || (el.scopedSlots = {})
...
;return ret
      ...
      for (let key in handler.modifiers) {
        if (modifierCode[key]) {
          ...
          genModifierCode += genGuard(
            ['ctrl', 'shift', 'alt', 'meta']
              .filter((keyModifier) => { return !modifiers[keyModifier]; })
              .map((keyModifier) => { return ("$event." + keyModifier + "Key"); })
              .join('||')
          )
        } else {
          keys.push(key)
  ...
  function genScopedSlots (
    ...
    let generatedSlots = Object.keys(slots)
      .map((key) => { return genScopedSlot(slots[key], state); })
      .join(',')
  ...
  function genScopedSlot (
    ...
    let slotScope = el.slotScope === emptySlotScopeToken
      ? ""
      : String(el.slotScope)
  ...
  function getNormalizationType (
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (needsNormalization(el) ||
          (el.ifConditions && el.ifConditions.some((c) => { return needsNormalization(c.block); }))) {
        res = 2
        break
      }
      if (maybeComponent(el) ||
          (el.ifConditions && el.ifConditions.some((c) => { return maybeComponent(c.block); }))) {
        res = 1
      }
    }
    return res
  ...
  function genSlot (el, state) {
    ...
    let attrs = el.attrs || el.dynamicAttrs
      ? genProps((el.attrs || []).concat(el.dynamicAttrs || []).map((attr) => { return ({
    ...
    return function compileToFunctions (
      ...
      // check compilation errors/tips
      {
        ...
        if (compiled.tips && compiled.tips.length) {
          if (options.outputSourceRange) {
            compiled.tips.forEach((e) => { return tip(e.msg, vm); });
          } else {
            compiled.tips.forEach((msg) => { return tip(msg, vm); });
          }
        }
      }
  ...
  let createCompiler = createCompilerCreator((
    template,
    options
  ) => {
